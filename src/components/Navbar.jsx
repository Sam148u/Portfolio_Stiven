...
function Navbar() {
  ...
  const getNavbarOffset = useCallback(() => {
    const navBar = navRef.current?.querySelector("[data-navbar-bar]")
  ...
  const scrollToSection = useCallback((href) => {
    ...
    const targetPosition = Math.max(
      sectionPosition - getNavbarOffset(),
      0,
    )
    ...
    lastScrollYRef.current = targetPosition
  ...
  const handleDesktopNavigation = useCallback((event, href) => {
    event.preventDefault()
    scrollToSection(href)
  }, [scrollToSection])

  const handleMobileNavigation = useCallback((event, href) => {
    event.preventDefault()
    setShowNavbar(true)
    setMenuOpen(false)
    window.clearTimeout(pendingScrollTimeoutRef.current)
  ...
  return (
    <div className="fixed top-4 left-1/2 z-50 w-[95%] max-w-7xl -translate-x-1/2">
      <motion.nav
        ...
        className="w-full overflow-hidden rounded-2xl border border-green-500/25 bg-black/45 shadow-[0_0_28px_rgba(34,197,94,0.16),0_0_60px_rgba(168,85,247,0.1)] backdrop-blur-xl"
      >
              ...
              {NAV_ITEMS.map((item) => (
                <li key={item.href}>
                  <a
                    ...
                    className="block w-full text-left transition-colors duration-200 hover:text-green-400"
                  >
                    {item.label}
                  </a>
                </li>
